...
function btnClick(obj) {
    if (obj.className.includes("insert")) {
        document.getElementById("modalTitle").innerHTML = "Yazı Ekle";
        document.getElementById("btnSumbit").innerHTML = "Yeni Yazı Kaydet";
        FormClean();
    }
    else if (obj.className.includes("edit")) {
        document.getElementById("modalTitle").innerHTML = "Yazı Güncelleme";
        document.getElementById("btnSumbit").innerHTML = "Yazı Güncelle";
        Find(obj.id);
    }
}
function FormClean() {
...
function PostData(obj) {
    ...
    if (obj.innerHTML == "Yazı Güncelle") {
        if (Imgdata.length > 0) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.state) {
                    ...
                    Swal.fire({
                        title: "Yazı Güncelleme!",
                        text: "Yazı başarıyla güncellenmiştir.",
                        type: "success"
                    })
                    PageLoad();
                }
                    ...
                    Swal.fire({
                        title: "Yazı Güncelleme!",
                        text: "Yazı bilgilerini lütfen kontrol ediniz. İşlem tamamlanamadı",
                        type: "warning"
                    })
                }
            }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.state) {
                    ...
                    Swal.fire({
                        title: "Yazı Kaydetme!",
                        text: "Yazı başarıyla kaydedildi.",
                        type: "success"
                    })
                    PageLoad();
                }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.state) {
                    ...
                    document.getElementsByClassName("modal-backdrop")[0].hidden = true;
                    Swal.fire({
                        title: "Yazı İçerik Güncelleme!",
                        text: "Yazı İçerik başarıyla güncellenmiştir.",
                        type: "success"
                    })
                    PageLoad();
                }
                else {
                    Swal.fire("Yazı İçerik Günceleme!", x.message, "warning");
                }
            },
            error: function (x) {
                Swal.fire("Yazı İçerik Günceleme!", x.message, "error");
            }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.state) {
                    ...
                    Swal.fire("Yazı İçerik Ekleme!", x.message, "warning");
                }
            },
            error: function (x) {
                Swal.fire("Yazı İçerik Ekleme!", x.message, "error");
            }
        });
    }
}
