...
function btnClick(obj) {
    if (obj.className.includes("insert")) {
        ...
        document.getElementById("btnSumbit").innerHTML = "Slider Yazı Kaydet";
        FormClean();
    }
...
function PostData(obj) {
    ...
    if (obj.innerHTML == "Slider Güncelle") {
        if (Imgdata.length > 0) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.state) {
                    ...
                    Swal.fire({
                        title: "Slider Güncelleme!",
                        text: "Slider bilgilerini lütfen kontrol ediniz. İşlem tamamlanamadı",
                        type: "warning"
                    })
                }
            }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.state) {
                    ...
                    document.getElementsByClassName("modal-backdrop")[0].hidden = true;
                    Swal.fire({
                        title: "Slider Kaydetme!",
                        text: "Slider başarıyla kaydedildi.",
                        type: "success"
                    })
                    PageLoad();
                }
...
function SubPostData() {
    ...
    SubData.Description = document.getElementById("Description").value;
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.state) {
                    ...
                    document.getElementsByClassName("modal-backdrop")[0].hidden = true;
                    Swal.fire({
                        title: "Slider İçerik Güncelleme!",
                        text: "Slider İçerik başarıyla güncellenmiştir.",
                        type: "success"
                    })
                    PageLoad();
                }
                else {
                    Swal.fire("Slider İçerik Günceleme!", x.message, "warning");
                }
            },
            error: function (x) {
                Swal.fire("Slider İçerik Günceleme!", x.message, "error");
            }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.state) {
                    ...
                    Swal.fire("Slider İçerik Ekleme!", x.message, "warning");
                }
            },
            error: function (x) {
                Swal.fire("Slider İçerik Ekleme!", x.message, "error");
            }
        });
    }
}
